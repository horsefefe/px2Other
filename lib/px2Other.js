...
var defaultConfig = {
  ...
  vminUnit: 7.5, // vmin unit
  convert_unit: "vmin",
  unit: "px" // base unit could be upx
};

function Px2Other(options) {
  this.config = {};
  extend(this.config, defaultConfig, options);
  let regText = `\\b(\\d+(\\.\\d+)?)${this.config.unit}\\b`;
  this.pxRegExp = new RegExp(regText);
}

Px2Other.prototype.convert = function (cssText) {
  var self = this;
...
Px2Other.prototype._getCalcValue = function (type, value) {
  var config = this.config;
  var pxGlobalRegExp = new RegExp(this.pxRegExp.source, "g");
  ...
  return value.replace(pxGlobalRegExp, function ($0, $1) {
    if (type === "px") {
      return getValue($1 / config.baseDpr);
    } else if (type === "rem") {
      return getValue($1 / config.remUnit);
    } else if (type === "vmin") {
      return getValue($1 / config.vminUnit);
    }
  });
};

module.exports = Px2Other;
